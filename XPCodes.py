...
def SXSZXZ(SZ='',SX='',XZ='',N=2):
    ...
    if len(SX) > 0:
        SX = str2ZMatdelim(SX)
        XZ = str2ZMatdelim(XZ) if len(XZ) > 0 else 0
        SX = makeXP(0,SX,XZ)
        SX = [XPSetEval(A,N) for A in SX]
        S = SX if S is None else np.vstack([S,SX])
    return S
...
def vDiv(A,b,N):
    ...
    # print(func_name(),s)
    A = np.reshape(A,-1)
    # print(func_name(),A,b,N)
    A = [Div(a,b,N) for a in A]
    A = np.reshape(A,s)
    # print(func_name(),np.shape(A))
    return A
...
def getEm(S,N,check=False,display=False):
    n = XPn(S)
    ...
    ## check if all phases are zero
    pZero = isZero(XPp(SZ))
    ## if so, Szp is the Z-component; otherwise, its both Z and phase components
    Szp = XPz(SZ) if pZero else XP2Zp(SZ,N,double=False)
    # report('Szp')
    ...
    ## If we had a non-trivial phase component, include only elements with 1 in -1 position
    if not pZero:
...
def Orbit2dist(Eq,SX,t=None):
    ...
    for q in range(len(Eq)):
        ...
        for k in range(1,t+1):
            for x in itertools.combinations(range(r),k):
                # print('x',x)
                # print('SX[x]',SX[x])
                temp.append(np.mod(Eq[q] + np.sum(SX[x,:],axis=0),2))
    return ZMat(temp)

## return next row of A
## return the rows such that a.a mod 2 = 1 first
## then from top of the list
def gsNext(A):
    ## if there are no rows, return False
    if len(A) == 0:
        return False
    for i in range(len(A)):
        ## check if dot product of row with itself is 1
        if np.sum(A[i]) % 2 == 1:
            return i 
    for i in range(len(A)-1):
        for j in range(i+1,len(A)):
            if np.sum(A[i] * A[j]) % 2 > 0:
                return i
    return False

## generate orthogonal set of vectors from A
def gramSchmidtZ2(A):
    # print(func_name(), np.mod(np.sum(A,axis=-1),2))
    temp = []
    ## convert A to a list of rows
    A = [a for a in A]
    ## get next row
    i = gsNext(A)
    while i is not False:
        ## remove a from A
        a = A.pop(i)
        # print('a',np.sum(a) % 2, a)
        ## rows which have dot product 1 with a
        ix = [j for j in range(len(A)) if np.sum(A[j]*a) % 2 > 0]
        ## update these rows
        for j in ix:
            A[j] = np.mod(A[j] + a,2)
        temp.append(a)
        i = gsNext(A)
    return ZMat(temp),ZMat(A)

## for PSF code get LXx
def PSFgetLXx(SXx, SZz):
    m,n = np.shape(SXx)
    LiX = leadingIndices(SXx)
    mask = 1 - set2Bin(n,LiX)
    SZz = SZz * mask
    nsp = NSpace(SZz,2)
    SZz = nsp.H
    LiZ = leadingIndices(SZz)
    ...
    for i in range(k):
        LXx[i][LiE[i]] = 1
        for j in range(nkr):
            LXx[i][LiZ[j]] = SZz[j][LiE[i]]
    LXx, rowops = How(LXx,2)
    return LXx

## get diagonal logical identity generators
def getMZzp(Eq,SXx, LXx,N):
    t = logCeil(N-1,2) 
    t = t if 2 ** t == N else None
    # print(func_name())
    # print('t',t)
    A = Orbit2dist(Eq, np.vstack([SXx,LXx]), t)
    # print('A',len(A))
...
def spanGens(vList,N):
    vList = ZMat(vList)
    j = argmin(np.sum(vList,axis=-1))
    b = vList[j]
    vGrp = np.mod(vList - b, N)
    H,rowops = How(vGrp,N)
    b, u = matResidual(H, b, N)
    return H, b

def commNaive(SXx,MZzp,N):
    r,n = np.shape(SXx)
    p = MZzp[:,-1]
    MZz = MZzp[:,:-1]
    ...
    for z in itertools.product(range(N),repeat=n):
        for i in range(r):
            x = SXx[i]
            xz = x * z
            zi,u = matResidual(MZz,np.mod(-2 * xz,N),N)
            if np.sum(zi) == 0 and (np.sum(xz) - np.sum(p*u)) % N == 0:
                temp[i].append(z)
    for i in range(r):
        temp[i] = spanGens(temp[i],N)
    return temp


## span of commutators of x up to operators in MZzp
def commZ(x,MZzp,N):
    x = np.hstack([x,[1]])
    n = len(x)
    ## diag(x)
    Ix = ZMat([set2Bin(n,[i]) for i in range(n) if x[i] == 1])
    ## Ker([x]) restricted to cols where x==1
    ## drop last row
    Kx = Ix.copy()[:-1]
    ## set final col (phase) to N-1
    Kx[:,-1] = N - 1
    ## find z such that 2xz in MZzp
    Rx = 2 * Ix
    Rx = nsIntersection([MZzp,Rx],N)//2
    ## adjust phase component
    Rx[:,-1] = 2 * Rx[:,-1]
    ## adding N//2 where x is set also results in solutions
    Rx = np.vstack([Rx, Ix[:-1] * N//2 ])
    # print('Rx')
    # print(ZmatPrint(Rx))
    # print('Kx')
    # print(ZmatPrint(Kx))
    Rx = nsIntersection([Kx,Rx],N)
    Ix = ZMat([set2Bin(n,[i]) for i in range(n) if x[i] == 0 or i == n-1])
    Rx = np.vstack([Rx,Ix])
    # print('Rx n Kx')
    # print(ZmatPrint(Rx))
    return Rx

## find generators of diagonal LO, independent of LXx
## assume N is even

def DiagLO(SXx,MZzp,N):
    R = None
    r,n = np.shape(SXx)
    t = logCeil(N) - 1
    print(func_name(),f't={t}')
    if 1 << t != N:
        t = None
    SXx = Orbit2dist(ZMatZeros((1,n)), SXx,t)
    ...
    for i in range(len(SXx)):
        ...
        if R is None:
            ...
            R = nsIntersection([R,Rx],N)
    # print('R Intersection')
    # print(ZmatPrint(R))
    MZz = MZzp[:,:-1]
    pList = MZzp[:,-1]
    R = R[:,:-1]
    temp = []
    for z in R:
        z1,u = matResidual(MZz,z,N)
        if np.sum(z1) > 0:
            temp.append(z1)
    nsp = NSpace(temp,N)
    LZz = nsp.H
    for r in LZz:
        # print('r',r,np.sum(r) % N)
        for x in SXx:
            rx =  r * x
            z,u = matResidual(MZz,2*rx,N)
            if np.sum(z) > 0:
                print('error: 2xz not in MZz',x,r)
            if (np.sum(rx) - np.sum(pList * u)) % N > 0:
                print('error: x.z != u.p',x,r)
    return LZz

## print clifford hierarchy operator
def vec2CPstr(v,LISet,N):
    # print(f'v: {v}')
    ## phase applied
    p = max(v)
    if p ==0:
        return "I"
    ## core/logical indices of all CW which have non-zero phases
    temp = None
    for i in range(len(v)):
        if v[i] > 0:
            ...
            if temp is None:
                ...
                temp *= ix
    logical = ZMat(bin2Set(temp))
    g = np.gcd(p,N)
    p,N = p//g, N//g
    return f'CP({p}/{N}; {logical})'

## clifford operators up to level t
def CliffordLO(Eq,LXx,SXx,t=1,target=None):
    N = 1 << t
    k = len(LXx)
    q = len(Eq)
    r = len(SXx)
    n = len(Eq[0])
    # Index Eq by core number
    # Eq = np.hstack([Eq,np.eye(q,dtype=int),ZMatZeros((q,k))])
    # ## Index LXx by logical index
    # LXx = np.hstack([LXx,ZMatZeros((k,q)),np.eye(k,dtype=int)])
    # ## Adding SX does not change logical or core index
    # SXx = np.hstack([SXx,ZMatZeros((r,k+q))])

    # Assume EQ is Zero
    Eq = ZMatZeros((1,n+k))
    ## Index LXx by logical index
    LXx = np.hstack([np.eye(k,dtype=int),LXx])
    ## Adding SX does not change logical or core index
    SXx = np.hstack([ZMatZeros((r,k)),SXx])
    ...
    # print('LI')
    if target is None:
        LI = [tuple(r) for r in LI]
        LISet = sorted(set(LI))
        LIn = len(LISet)
        LIDict = {LISet[i]: set2Bin(LIn,[i]) for i in range(LIn)}
        LI = ZMat([LIDict[r] for r in LI])
    else:
        target = set2Bin(k,target)
        LISet = [tuple([0]*k),target]
        LIn = 2
        LI = ZMat([[0,1] if sum(r * target -target) == 0 else [1,0] for r in LI])
    # LI = LI[:,1:]
    # print(LI)
    A = np.hstack([LI,A])
    nsp = NSpace(A,N)
    nsp.simplifyKer()
    # print(f'K\n{ZmatPrint(nsp.K)}')
    LE,LO = nsp.K[:,:LIn],nsp.K[:,LIn:]
    return LO, LE
    return [vec2CPstr(v,LISet,N) for v in LE if sum(v)>0], LO

    # Li = ZMat(leadingIndices(nsp.K))
    # return nsp.K[Li < LIn,:],LISet
    # return nsp.K
...
class Code:
    ...
    def getEq(self):
        if self.N > 2:
            ...
            S = getVal(self,'S')
            SX,SZ = splitDiag(S)
            SXx, SZz = XPx(SX),XPz(SZ)
            LXx = PSFgetLXx(SXx, SZz)
            SZp = XP2Zp(SZ,2,double=False)
            nsp = NSpace(SZp,2)
            nsp.simplifyKer()
            K = [a for a in nsp.K if a[-1]==1]
            Eq,u = matResidual(np.vstack([SXx,LXx]),K[0][:-1],2)
            Eq = ZMat([Eq],self.n)
    ...
    def getCWL(self,P = None):
        ...
        # print(func_name(),EmState,SX)
        return getCWP(EmState,SX,self.N,P)

    def getLI(self,P=None):
        if P is None:
            P = self.N
        MX,Eq,SXx,LXx = getVals(self,['SX','Eq','SXx','LXx'])
        if P != self.N:
            MX = XPSetN(MX,self.N,P)
            if MX is False:
                return False
        MZzp = 2 * getMZzp(Eq,SXx,LXx,P)
        MZ = Zp2XP(MZzp,P)
        for i in range(len(MX)):
            p,x,z = XPcomponents(MX[i])
            zp,u = matResidual(MZzp,np.hstack([2*z, p]),2*P)
            p, z = zp[-1],zp[:-1]//2
            MX[i] = makeXP(p,x,z)
        return np.vstack([MX,MZ])
    ...
    def getLD(self):
        N = self.N
        S,L,EmState,wI = getVals(self,['S','LO','EmState','wI'])
...
def MeasureCodewords(CW,A,N):
    # print(func_name(),'A',A,'N',N)
    # print('CW')
    # print(ZmatPrint(CW,N))
    # # report(func_name(),CW)
    n = XPn(A)
    nZero = ZMatZeros(n)
...
def addGraphNode(nodes,ix,Li):
    j,p = ix
    if j not in nodes:
        nodes[j] = dict()
    label = str(ZMat2str(p))
    l = Li[j] if j < len(Li) else len(label)
    label = '*'*l + label[l:]
    nodes[j][p] = {'label':label,'x':1000,'solutions':0}

## display generic graph
def displayGraph(D,ix,title,K,save=False):
    dpi = 300
    matplotlib.pyplot.figure(dpi=dpi)
    Li = leadingIndices(K)
    m,n = np.shape(K)
    toDo = [ix]
    nodes = dict()
    addGraphNode(nodes,ix,Li)
    nodes[ix[0]][ix[1]]['solutions'] = 1
    edges = dict()
    while len(toDo) > 0:
        ix = toDo.pop(0)
        j,p = ix
        if len(D[ix]) > 0:
            for d,p1 in D[ix].items(): 
                ...
                if p1 is not None:
                    ...
                    if ix2 not in toDo:
                        toDo.append(ix2)
                        addGraphNode(nodes,ix2,Li)
                    x = d + nodes[j][p]['x']
                    nodes[j1][p1]['x'] = min([x,nodes[j1][p1]['x']])
                    ...
                    if j1 not in nodes:
                        addGraphNode(nodes,ix2,Li)
                nodes[j1][p1]['solutions'] += nodes[j][p]['solutions']
                eix = (j,p,p1)
                if eix not in edges:
                    edges[eix] = []
                edges[eix].append(str(d))
    nMax = max(nodes.keys())
    for j,nList in nodes.items():
        pList = list(nList.keys())
        xList = [nList[p]['x'] for p in pList]
        ix = argsort(xList)
        for i in ix:
            nList[pList[i]]['x'] = i 
            nList[pList[i]]['y'] = nMax - j
    ## plot edges
    eList = [((nodes[n][p]['x'],nodes[n][p]['y']),(nodes[n+1][p1]['x'],nodes[n+1][p1]['y'])) for (n,p,p1) in edges.keys()]
    lc = mc.LineCollection(eList, linewidths=0.5,colors='gray')
    fig, ax = pl.subplots()
    ...
    for (j,p,p1),dList in edges.items():
        node1,node2 = nodes[j][p],nodes[j+1][p1]
        xCoord,yCoord = (node1['x']+node2['x'])/2, (node1['y']+node2['y'])/2
        # ax.annotate("/".join(dList), (xCoord + xoffset, yCoord))
        ax.text(xCoord + xoffset, yCoord,"/".join(dList), ha="left", va="center", size=7)
    for j,nList in nodes.items():
        for p,pNode in nList.items():
            xCoord = pNode['x']
            yCoord = pNode['y']
            ax.scatter(xCoord, yCoord,c='dimgray',s=4)
            # ax.annotate(pNode['label'], (xCoord - xoffset, yCoord))
            ax.text(xCoord, yCoord, pNode['label'], ha="center", va="center", size=7, bbox=dict(boxstyle="Round,pad=0.3",fc="white", ec="dimgrey", lw=1))
            if j == nMax:
                title = title + f'\nSolution Count: {pNode["solutions"]}'
    plt.title(title)
    if save:
        global plotNo
